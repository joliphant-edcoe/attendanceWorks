...
CUTOFF_DATE = datetime.datetime(2024,11,1)
FILENAME_DATE = 'Nov24'
YEAR_CODE1 = '2425'
YEAR_CODE2 = str(int(YEAR_CODE1[:2])-1) + str(int(YEAR_CODE1[-2:])-1)  #'2324'
YEAR_CODE3 = str(int(YEAR_CODE1[:2])-2) + str(int(YEAR_CODE1[-2:])-2)  #'2223'
...
for i, d in enumerate(districts):
    ...
    if d == 'EDCOE_charter':
        # special case for tiered intervention letters for EDCOE charter
        tier = os.path.join(path, 'tiered_intervention.xlsx')
        truancy = AttendanceTruancySupp(current_stu_datafile, school_type=district_type[i], tier_file=tier)
    else:
